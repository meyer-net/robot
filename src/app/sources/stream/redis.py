...
class Redis(SourceBase):
    ...
    def get_handler(self):
        ...
        return StoreCacheRedis(**args)

    def get_position_key(self):
        args = self._args
        return "{source_key}${ref}_position".format(**args)
    ...
    def mount(self, handler, ctx):
        ...
        '''
        mq_key, mq_elements = handler.read_buffer(self._args["source_key"])
        return self._collect_from_elements(ctx, mq_key, mq_elements)

    def _collect_from_elements(self, ctx, mq_key, mq_elements):
        '''
        内部函数，用于收集来自于适配器中的数据
        '''
        if mq_key != None:
